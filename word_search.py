...
def search_desc(entry):
    ...
    res_list = [dryh, lact, clone, juicy, dank, wedding, whisk, trop,award, best, smoke, roast, thick,heavy, stfruit, hazy, pheno,\
                ...
                malt, agg, balance, complex, crisp, smooth, diac, apple, floral, fruity, robust, wood, vanilla, cherry, funk, grapef,solvent]
    resparray = np.asarray(res_list)
    response = pd.DataFrame(resparray, index = ['dryh', 'lact', 'clone', 'juicy',\
                                                'dank', 'wedding', 'whisk', 'trop','award', 'best', \
                                                'smoke', 'roast', 'thick','heavy', 'stfruit', 'hazy', 'pheno',\
                                                'banana','clove', 'bitter','puree','lovehop','strong', 'dry', \
                                                'sweet', 'sour', 'dark', 'pine', 'sticky', 'foam', 'hoppy',\
                                                'bisc', 'ester', 'straw', 'choco', 'coffee'\
                                                'malt', 'agg', 'balance', 'complex', 'crisp', 'smooth', 'diac', 'apple', 'floral', 'fruity',\
